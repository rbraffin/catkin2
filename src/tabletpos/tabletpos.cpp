/*
  ...
  Copyright (c) 2016 Walter Fetter Lages <w.fetter@ieee.org>
	...
	Alterado para cadeira de Robotica A, com fins educacionais.
...
{
	ros::init(argc, argv, "tablet2node");
	ros::NodeHandle pxyp;
	ros::Rate loop_rate(10);
	while (ros::ok())
	ros::Publisher pub_xyp = pxyp.advertise<std_msgs::Float32MultiArray>("xypressure", 1000);
	{
	...
	{		
		...
		std_msgs::Float32MultiArray xyp[3]={};
		xyp [0]=ev_pkt.x;
		xyp [1]=ev_pkt.y;
		xyp [2]=ev_pkt.pressure;
		pub_xyp.publish(xyp);
		switch(ev_pkt.type)
		{
